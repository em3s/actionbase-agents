...
PR/이슈 본문, 코멘트 등 모든 산문 산출물에 적용. 목표: 개발자가 직접 쓴 글처럼 읽히는 것.

- 변경의 동기가 된 문제·관찰로 시작 (왜부터). 제목 재진술 여는 문장 금지
- 분량은 변경 크기에 비례 — 사소한 수정은 헤더 없이 한두 문장, 빈 섹션은 삭제 또는 "None.", 마무리 요약 문단 금지
- 1인칭·능동, 짧은 평서문, 조각문 허용. 검증한 사실은 단정, 검증 안 한 것만 밝힌다
- 트레이드오프는 단점 한 문장 + 재방문 조건. 미룬 일은 명시: "I'll leave that for a future PR."
- **인위적 줄바꿈 금지 — 한 문단 = 한 줄.** 문단은 1–4문장
- 불릿은 3–6개 짧은 명령형, diff 파일별 재나열 금지, 볼드 접두 불릿 금지
- 증거는 서술 대신 원본 붙여넣기 (로그 fenced block, CI는 bare URL 한 줄)
- 금지: 마케팅 형용사 (robust, comprehensive / 강력한, 포괄적인), 이모지 불릿, AI 귀속 푸터
- 코멘트는 짧고 따뜻하고 단정적으로. 실수는 즉시 인정
